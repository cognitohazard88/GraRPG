...
class Fight{
    public void StartFight(Character enemy, Character hero, Scanner scan, Inventory inventory){
        ...
        while (heroHealth > 0 && enemyHealth > 0) {
            while(turn == turn){
                ...
                if (x.equals("A")) {
                    attackList(scan, hero, enemy, turn);
                } else if (x.equals("I")) {
    ...
    public void attackList(Scanner sc, Character character, Character enemy, int turn){
        while(true){
            ...
            if (attack.equals("A")) {
                character.attack(character, enemy, turn);
                break;
            } else if (attack.equals("S")) {
                if (turn >= 3) {
                    character.specialAttack(character, enemy, turn);
                    break;
                } else {
